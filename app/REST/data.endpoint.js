const posts = [
 ...
 const dataEndpoint = (router) => {
    router.get('/api/posts', async (request, response, next) => {
        if (request.query.title) {
            response.status(200).send(
                {posts: posts.filter(el => el.title.includes(request.query.title))}
            );
        } else {
    ...
    router.get('/api/posts/:page/:limit', async (request, response, next) => {
        ...
        response.status(200).send({posts: postsArr});
    });

    router.post('/api/posts', async (request, response, next) => {
        if (request.body?.newPost?.id && request.body?.newPost?.title && request.body?.newPost?.text && request.body?.newPost?.image) {
    ...
    router.get('/api/post/:id', async (request, response, next) => {
        const i = Number(request.params.id) - 1;
    ...
    router.put('/api/post/:id', async (request, response, next) => {
        ...
        posts[i] = {
            id: request.body.newPost.id ?? posts[i].id,
            title: request.body.newPost.title ?? posts[i].title,
            text: request.body.newPost.text ?? posts[i].text,
            image: request.body.newPost.image ?? posts[i].image,
        };
